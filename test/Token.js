...
describe('Token', () => {
  ...
  describe('Approving Tokens', () => {
    ...
    describe('Success', () => {
      ...
      it('emits an Approval event', async () => {
        const approvalEvent = result.logs[0];

        const { args } = approvalEvent;
        expect(args[0]).to.equal(deployer.address);
  ...
  describe('Delegated Token Transfers', () => {
    ...
    describe('Success', () => {
      ...
      it('emits a Transfer event', async () => {
        const delegatedtransferEvent = result.logs[0];

        const { args } = delegatedtransferEvent;
        expect(args[0]).to.equal(deployer.address);
        expect(args[1]).to.equal(receiver.address);
        expect(args[2]).to.equal(amount);
      })
